...
class BookService:
    ...
    def add_book(self, book: Book):
        """Thêm sách mới"""
        # Quy tắc: Kiểm tra mã sách có bị trùng không [cite: 160]
    ...
    def get_all_books(self):
        """Xem danh sách sách [cite: 161]"""
        return self.book_repo.get_all()

    def search_books(self, keyword: str):
        """Tìm kiếm sách theo tên, tác giả, thể loại [cite: 164]"""
    ...
    def delete_book(self, book_code: str):
        ...
        # Quy tắc: Không cho xóa sách nếu sách đang được thuê [cite: 201]
        if book.BookStatus == 'Rented':
            raise ValueError("Từ chối xóa: Sách đang được thuê[cite: 168].")
            
        return self.book_repo.delete_book(book_code)

    def update_book_status(self, book_code: str, new_status: str):
        """Cập nhật trạng thái sách thủ công [cite: 170]"""
